...
function spicepress_fallback_page_menu( $args = array() ) {
	...
	if ( ! empty($args['show_home']) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = esc_html__('Home','spicepress');
		else
			$text = $args['show_home'];
		$class = '';
	...
	$list_args['title_li'] = '';
	$list_args['walker'] = new Spicepress_walker_page_menu;
...
class Spicepress_walker_page_menu extends Walker_Page{
	...
	function start_el( &$output, $page, $depth=0, $args = array(), $current_page = 0 ) {
		...
		$css_class = array('menu-item page_item', 'page-item-'.$page->ID);
                if ( class_exists( 'WooCommerce' ) && is_shop() ) {
                    $current_page = woocommerce_get_page_id('shop');
                }
